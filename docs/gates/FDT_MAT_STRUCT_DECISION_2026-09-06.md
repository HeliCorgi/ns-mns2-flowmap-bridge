...
Differentiating (0.4) gives the operator equation
...
Hence the material projector is itself a Fourier multiplier with symbol
...
Moreover, among differentiable operator families with initial value `Delta_j` satisfying the exact
commutation equation (1.1), the conjugated family (0.4) is the canonical solution; in the affine
Fourier-multiplier class (2.3) is forced by the transport equation for the symbol.
...
Use the same one-window length as the parent FDT gate,
...
Then over exactly one viscous window,
...
Viscous time localization therefore does not restore a universal comparison between material and
Eulerian dyadic indices.

This is the same mechanism that made the affine low–high commutator large, now expressed in the
co-moving frequency frame.

---

## 5. Exact failure of the naive continuation bridge

Consider a passively transported mode in the compressive `x_1` direction.  Its Eulerian frequency
obeys
...
> all sufficiently high material blocks are subcritical

need not imply

> all sufficiently high Eulerian blocks are subcritical

with a uniform index conversion independent of deformation.

The Cheskidov–Shvydkoy dissipation wavenumber and the parent `FDT-INJ` first-contact theorem are
formulated in fixed Eulerian dyadic shells.  To convert a material-tail estimate back to that
criterion one needs quantitative control of
...
assumption would therefore reintroduce the continuation wall unless an independent budget is proved.
...
There is a second structural price which is independent of the shell-distortion no-go.
...
componentwise composition does **not** preserve the divergence-free condition of a vector field.
Thus applying Leray after material conjugation creates a geometric/Leray defect.
...
which does preserve divergence when `det DX=1`.  But its natural transport equation is the Lie
transport equation; relative to the Navier–Stokes componentwise convection it introduces the
explicit deformation term
...
3. attempt to project back with Leray, creating a nonlocal commutator with the transported geometry.

No option removes low-frequency deformation for free.  This does not by itself prove a separate
impossibility theorem for every conceivable material observable, but it identifies the residual that
must be paid after (1.2).
...
The reason is exact and structural: items 1–3 force transported/deformed frequency shells, while the
affine trace-free strain makes item 4 fail by an arbitrarily large factor over one viscous window.
Without item 4, item 5 cannot be obtained from the fixed-Eulerian Cheskidov–Shvydkoy criterion by a
uniform shell comparison.

At the same time the positive sub-result
...
- a new continuation theorem formulated directly in a deformation-aware metric rather than fixed
  Eulerian dyadic shells;
...
- an admissible localization theorem showing whether the affine shell-distortion obstruction can be
  approximated by finite-energy Schwartz data over one window.

The affine countermodel is still non-`L^2`, so this record is **not** a Clay-admissible counterexample
and does not refute `FDT-INJ`.

---

## 9. Strategic ruling

The current frequency route has now exposed two independent facts:
...
Therefore the next useful decision should not add more LP/paracomposition plumbing.  Two honest
options remain:

### Option A — `FDT-DEF-BUDGET`
Ask whether the actual NS low-frequency flow has an independently finite/subcritical deformation
budget over viscous windows strong enough to compare material and Eulerian shells, **without**
reducing to `int F`, `int ||grad u||_infinity`, Serrin, or bounded `H^3`.

This should be attacked counterexample-first.  If NO, park the material-frequency regularity route.

### Option B — `FDT-MAT-CONT`
Abandon the fixed Eulerian continuation criterion and attempt to prove a genuinely new continuation
theorem directly in the deformation-aware material metric.  Such a theorem must still recover
classical regularity and cannot hide deformation control in the norm definition.

Option A has the higher information value and should be attempted first.
...
The exact commutation identity is elementary, but formalizing flow-conjugated LP projectors before a
surviving analytic continuation bridge exists would be plumbing without proof-frontier value.  The
standing rule remains: no LP/Bony/paracomposition Lean layer until a load-bearing analytic gate
survives counterexample-first testing.
