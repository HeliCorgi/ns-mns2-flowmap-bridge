...
Fourier convention.
-/
def R3HsVelocity (_s : ℝ) := R3L2Velocity
...
def r3HsToTemperedCLM (s : ℝ) :
  ...
  TemperedDistribution.besselPotential R3 R3C (-s) ∘L
...
theorem r3HsToTemperedCLM_apply (s : ℝ) (f : R3HsVelocity s) :
    r3HsToTemperedCLM s f =
      TemperedDistribution.besselPotential R3 R3C (-s)
        ((f : R3L2Velocity) : 𝓢'(R3, R3C)) := by
  rfl
...
theorem r3HsToTempered_memSobolev
  ...
  refine ⟨(f : R3L2Velocity), ?_⟩
  exact besselPotential_r3HsToTempered_eq_coordinate s f
