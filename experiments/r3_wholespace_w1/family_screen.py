#!/usr/bin/env python3
"""Preregistered alpha-kappa family screen for the R3 whole-space track.

Implements docs/gates/R3_AXISYM_WHOLESPACE_FAMILY_SCREEN_PREREG_2026-09-09.md.
This is finite-resolution triage only; it makes no singularity or Clay claim.
"""
...
H = 0.08
DT = 0.002
ALPHAS: Tuple[float, ...] = (16.0, 32.0, 64.0, 128.0)
KAPPAS: Tuple[float, ...] = (0.75, 1.00, 1.50)
...
class FamilyGridSolver(gq.DomainGridSolver):
    ...
    def set_parameters(self, alpha: float, kappa: float) -> None:
        self.alpha = float(alpha)
        self.kappa = float(kappa)
        self.amplitude = self.alpha * v1.NU

    def initial_state(self):
        zscaled = self.zz / self.kappa
        u = self.amplitude * v1.bump(self.rr**2) * zscaled * v1.bump(zscaled**2)
        w = np.zeros_like(u)
        self.zero_outer(u)
        self.zero_outer(w)
        return u, w
...
def run_one(s: FamilyGridSolver, alpha: float, kappa: float, box_name: str) -> Dict[str, object]:
    s.set_parameters(alpha, kappa)
    v1.T_FINAL = TSCREEN
    v1.DT = DT
    print(f"row alpha={alpha:g} kappa={kappa:g} {box_name} h={H} dt={DT} T={TSCREEN}", flush=True)
    ...
    row: Dict[str, object] = {
        ...
        "diagnostics": {
            ...
            "final_omega1_sup_over_A": float(omega1sup / max(s.amplitude, 1e-30)),
        },
    }
...
def run_gate() -> Dict[str, object]:
    solvers = {name: FamilyGridSolver(H, rmax, zmax) for name, rmax, zmax in BOXES}
    ...
    for alpha in ALPHAS:
        for kappa in KAPPAS:
            key = f"a{alpha:g}_k{kappa:g}"
            pair = {name: run_one(solvers[name], alpha, kappa, name) for name, _r, _z in BOXES}
            ...
            if not runtime_pass:
                ...
                valid_count += 1
                promotions.append({"alpha": alpha, "kappa": kappa, "trigger_enstrophy": bool(g_e), "trigger_vorticity": bool(g_w)})
            else:
            ...
            print(f"classification {key}: {classification} E=({e22:.6g},{e44:.6g}) W=({w22:.6g},{w44:.6g}) Sbox={sbox:.3g} Vbox={vbox:.3g}", flush=True)
            del pair
            gc.collect()

    if valid_count == 0:
        decision = "STOP_REPAIR_SCREEN"
        passed = False
    elif promotions:
    ...
    return {
        "schema": "r3-alpha-kappa-family-screen-v1",
        "frozen_parameters": {
            "nu": v1.NU,
            "R": 1.0,
            "alphas": list(ALPHAS),
        ...
        "decision": decision,
        "pass": passed,
        "claim_boundary": "coarse matched-box alpha-kappa triage through T=.25 only",
    }
...
def main() -> int:
    ...
    print(json.dumps({"decision": result["decision"], "promotions": result["promotion_set"], "output": str(args.output)}, indent=2))
