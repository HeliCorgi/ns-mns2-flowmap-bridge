...
revision: working tree on main after 098803b
  (new: Formal/R3ClassicalIncompressibility.lean; extended: Formal/AxiomAudit.lean)
toolchain: leanprover/lean4:v4.32.1
dependency manifest: committed lake-manifest.json; mathlib per lake-manifest.json
target scope: lake build Formal.R3ClassicalIncompressibility / Formal.AxiomAudit —
  pass, no new warnings
full scope: lake build (Formal.+ default target) — pass (8758 jobs; remaining warnings
...
axiom scope: Formal.AxiomAudit — pass; all six new audited declarations
  (contDiff_one_r3PhysicalRepresentative, hasFDerivAt_r3PhysicalRepresentative,
   r3RepresentativeDeriv_div_eq_zero, r3ClassicalDivergence_r3PhysicalRepresentative,
   r3PhysicalRepresentative_incompressible_of_mem_solenoidal,
   r3PhysicalRepresentative_hypotheses_nonvacuous)
  depend only on propext, Classical.choice, Quot.sound
GitHub Actions: not invoked (quota exhausted; hosted runs banned)
```

Previous gate (edge 1a): full `Formal.+` pass at 8757 jobs, both 1a declarations
standard-axioms-only (see 098803b).
