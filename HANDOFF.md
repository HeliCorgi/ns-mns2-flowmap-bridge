...
revision: working tree on main after 5ddce9c
  (new: Formal/R3ProjectedMomentumEquation.lean; doc-updated:
   Formal/R3ProjectedMomentumDuhamelInfrastructure.lean; extended: Formal/AxiomAudit.lean)
toolchain: leanprover/lean4:v4.32.1
dependency manifest: committed lake-manifest.json; mathlib per lake-manifest.json
target scope: lake build Formal.R3ProjectedMomentumEquation / Formal.AxiomAudit —
  pass, no new warnings (new file: zero errors, zero warnings)
full scope: lake build (Formal.+ default target) — pass (8764 jobs; remaining warnings
...
axiom scope: Formal.AxiomAudit — pass; the six audited 2b-ii.a-assembly declarations
  (continuousOn_r3MildMomentumIntegrand, inner_r3MildDecodedVelocity_eq_integral,
   r3MildDecodedVelocity_eq_integral, hasDerivAt_r3MildDecodedVelocity,
   r3EndpointSafeProjectedMild_momentum, exists_r3EndpointSafeProjectedMild_momentum)
  depend only on propext, Classical.choice, Quot.sound
scope note: the 2b-ii.a-assembly debt is retired — the infrastructure lemmas are
  consumed by the assembly; not claimed: pressure / the unprojected equation
  (2b-ii.b), classical pointwise time derivatives beyond L2, derivatives at t = 0 or
  t = T; solenoidality/reality are neither used nor asserted (projected equation on the
  complex carrier, not yet an NS statement)
...
infrastructure at 8763 jobs (5ddce9c) — all their audited declarations
