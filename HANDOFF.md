...
edges 2b-ii (momentum-equation semantics proper), 3 proper, 4, 5 remain open.

## Latest Lean verification

```text
runner: local Windows (Git Bash) process via Elan
revision: working tree on main after 62a879b
  (new: Formal/R3ProjectedMomentumDuhamelInfrastructure.lean; extended: Formal/AxiomAudit.lean)
toolchain: leanprover/lean4:v4.32.1
dependency manifest: committed lake-manifest.json; mathlib per lake-manifest.json
target scope: lake build Formal.R3ProjectedMomentumDuhamelInfrastructure / Formal.AxiomAudit —
  pass, no new warnings (new file: zero errors, zero warnings)
full scope: lake build (Formal.+ default target) — pass (8763 jobs; remaining warnings
...
axiom scope: Formal.AxiomAudit — pass; all eight audited 2b-ii.a-infrastructure
  declarations (r3L2ToTempered_r3H3LaplacianL2Operator, r3H3LaplacianL2Operator_stokes,
   r3H3LaplacianL2Operator_smoothing, hasDerivAt_inner_r3StokesL2Path,
   integral_inner_r3StokesL2Path, integral_triangle_swap,
   r3MildDecodedVelocity_duhamel,
   r3HelmholtzPressure_gradient_trajectoryConvection)
  depend only on propext, Classical.choice, Quot.sound
scope caveat: this pass ships INFRASTRUCTURE ONLY — the momentum-equation assembly
  (integral identity + FTC-2, named edge 2b-ii.a-assembly) is neither proved nor claimed;
  the shipped lemmas are not verified to be sufficient for it, and no module other than
  Formal/AxiomAudit.lean consumes them
...
(faa0f96); edge 2b-i at 8762 jobs (ec3517b, witness addendum 62a879b) — all their
audited declarations standard-axioms-only.
