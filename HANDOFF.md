...
Last updated: **2026-09-07 JST — R3 whole-space W0 passed; W1 stability preflight opened**.
...
## PR #108 — R3 W1 stability preflight — current smallest gate

PR #108 is stacked on PR #107:

```text
Research: preregister R3 W1 stability preflight
...
Primary integrator is **classical RK4**; comparison integrator is **SSPRK3**. Heun/RK2 is negative-control only. The detector scans a `1025 x 1025` frozen-coefficient centered advection-diffusion symbol and includes a conservative interior `L5` radial stress `3 nu / r_min`.

Manufactured rows:

```text
S1: pure centered advection — must detect Heun instability while RK4/SSPRK3 remain stable
S2: stable advection-diffusion row including L5 radial stress
S3: intentionally oversized step — RK4 and SSPRK3 must both reject
```

Implementation:

`experiments/r3_wholespace_w1/stability_preflight.py`.

Workflow:

`R3 W1 stability preflight`, run `34108056875`.

At the time this handoff entry was written, the run had started but its final result had not yet been recorded here. Inspect the exact run/artifact before proceeding.

### If W1 stability preflight passes

The next gate is **short-time manufactured nonlinear evolution only**, consuming the exact W0 identities
...
- call any W1 preflight a singularity result;
...
5. this file;
6. current `main`;
7. PR #107 and its W0 result artifact;
8. PR #108 and workflow run `34108056875` plus its artifact/logs;
9. Fable5 P0-A/B/C/E audit text.

Current smallest objective:

```text
resolve PR #108 W1 stability preflight result;
if PASS, freeze and implement the short-time manufactured nonlinear W1 gate.
```
