...
> consumption or proof commissioning). Baseline stays the 8775-job gate.
>
> **2026-09-02 (twelfth session): V-13 is COMMISSIONED and DISCHARGED —
> VERDICT: QUANTITATIVE-BRIDGE**
> (`docs/gates/HSEL_V13_DISCHARGE_2026-09-02.md`, RECORD-ONLY; one primary
> deep-read lane + main-loop re-derivations). **(i)** the pure-NS reduction
> of Li–Pan (h₀ ≡ 0, ρ₀ ≡ 0) is clean — no estimate excludes zero fields;
> viscosity normalized `μ = 1`, general-ν recovered by the recorded scaling
> `(T,M,M′,Q) ↦ (νT, ν⁻¹M, ν⁻¹M′, ν⁻³Q)`. **(ii)** the `s = 1, (q,p) =
> (4,2)` equality endpoint is the proof's NATIVE case — the Young-produced
> Grönwall exponent `2p/((1+s)p−3)` equals exactly 4 there; non-strict `≤`
> as printed; only `p = 3/(1+s)` is excluded (smallness branch); no
> interval-splitting needed. **(iii)** the proof is quantitative end to
> end: explicit energy/Grönwall chain to a PROVED `H^m` bound (no
> compactness, no contradiction, no BKM delegation); `F` = explicit
> double-exponential tower. **One substantive carry: the tacit datum
> `M′ = ‖ru₀^θ‖_{L∞}`** (consumed in Lemma 3.1(i) and the s = 1 J-step
> (3.22)), main-loop-verified NOT ball-uniform on the H³ ball (swirl torus:
> `Γ₀ ≲ MR^{1/2} → ∞`) — a far-field-weight escape window, and a standing
> class datum of the frozen corpus (B2's own `Γ₀`). **EV-1 quantifier
> refinement:** T-SRC′'s constant is `Q₀(ν,T,M,M′)` and the nu-layer
> conclusion is uniform on the two-parameter ball `(M,M′)`; per-datum ds is
> unrestricted. **The T-SRC′ → H-SEL|axisym chain is now
> published-and-quantitative end to end — the first head in the lineage
> whose entire downstream bridge is verified known math with tracked
...
>
> **2026-09-04 (twenty-second session): the T-VAR inequality session is
> COMMISSIONED and EXECUTED — VERDICT: ROUTE-KILLED (the wall-free
> general T-DIR lane)** (`docs/gates/HSEL_TVAR_INEQUALITY_SESSION_2026-09-04.md`;
> no Lean, no numerics; no new named sufficient condition; [D]-yields only).
> Commission: find ≥1 NEW exact-NS inequality contributing to T-VAR
> without full-enstrophy/BKM/Serrin budgets (reformulations, stronger
> sufficient conditions, det/trace/eigenvalue restatements, open-head
> assumptions do not count). **Search yields (all FAILCASE, wall-level):**
> I-1 the Lamb-vector form of the production tensor
> `𝒫_{ij} = −∫∂ᵢ∂ⱼu·(ω×u)dx` (pressure-free, new to the record); I-2 the
> uniform-in-e domination `(X_e′)₊ ≤ ‖P(ω×u)‖²₂/(2ν)` with the exact budget
> `∫‖P(ω×u)‖² = ∫‖∂_tu‖² + ν(E(T′)−E(0)) + ν²∫‖Δu‖²` (H²-wall); I-4 no
> uniform-in-e pointwise domination can be sub-wall
> (`avg_e(X_e′)₊ ≥ (E′)₊/3`). **THE STRUCTURAL OBSTRUCTION — the
> symmetric-sector collapse lemma (SSC) [D, proved]:** the
> tetrahedrally-equivariant sub-ball `𝔅_Γ(M)` (Γ = A₄ generated by
> `diag(1,−1,−1)`, `diag(−1,1,−1)`, the cyclic permutation) is NS-flow
> invariant (O(3)-covariance + uniqueness), non-empty and
> infinite-dimensional at every `M` (Γ-averaged Schwartz potentials), and
> on it every Gram-type tensor is scalar at every time
> (`G = (E/3)I`, `D = (‖∇²u‖²/3)I`, `𝒫 = −(∫ωᵀSω/3)I`, `R₋ = ((E′)₊/6)I`,
> `𝔑 = (TV₊(E)/6)I`). Hence VERBATIM on the sector: **T-VAR ⟺
> `TV₊(‖∇u‖²₂) ≤ 3Q₀` (full-enstrophy rise budget = the wall); T-DIR ⟺
> `∫‖∇u‖⁴₂dt ≤ 9Q₀` (the L⁴_tḢ¹ wall — the CFZ (4,2) criterion is the
> full-gradient criterion there, C4 fails); T-DIR-SPK ⟺
> `∫(∫ωᵀSω)₊ ≤ 3Q₀`; T-CONE ⟺ T-DET ⟺ `TV₊(E) ≤ 6Q₀`
> (`λ_min𝔑 = tr𝔑/3`, `det𝔑 = (tr𝔑/3)³` — the "strictly weaker than
> trace" claims are equalities there); T-GRAM's canonical direction is
> the whole sphere (= the probe's Taylor–Green triple degeneracy).** The
> T-CONE/T-DET "anti-equidistribution / codim-1 confinement" input is
> identically FALSE on the sector (`R₋ ∝ I`), so it cannot be an NS theorem
> on the ball. Dichotomy: uniform contributions are wall contributions
> (rejected/unavailable); anisotropy-conditional ones are vacuous on the
> sector and cannot close. **The wall-free general lane is ROUTE-KILLED;
> recommended PARK (user act).** Erratum annotations appended (dated, no
> silent repair) to the channel-search, quantifier-battery, T-VAR and
> T-CONE records. New standing battery item **SYM-test**: evaluate any
> ∃-direction / anisotropy head on `𝔅_Γ(M)` before commissioning. HR-1/HR-3′
...
with proof and the collapse table; §3 the dichotomy; §5 verdict +
recommendations); the appended errata at the end of
`HSEL_CHANNEL_SEARCH_2026-09-03.md`, `HSEL_TDIR_QUANTIFIER_BATTERY_2026-09-03.md`,
`HSEL_TVAR_PROOF_SESSION_2026-09-04.md`, `HSEL_TCONE_PROOF_SESSION_2026-09-04.md`.

- **State of the general lane: ROUTE-KILLED as a wall-free program.**
  T-DIR / T-DIR-SPK / T-GRAM / T-VAR / T-CONE / T-DET remain OPEN statements
  with the verified Zhang/CFZ bridge (nothing downstream is retracted), but
  by SSC each of them coincides verbatim with a full-enstrophy wall
  statement on the NS-invariant tetrahedrally-equivariant sub-ball, so no
  wall-free proof of any of them exists and their "sub-wall / strictly
  weaker than trace" positioning is withdrawn on that sector.
- **NOT to do (moot, do not start implicitly):** the T-DET proof session;
  the T-DET-targeted probe; any further "strictly weaker invariant"
  search inside this family (every Gram-type invariant collapses to its
  trace on the sector); any proof search on T-DIR/T-DIR-SPK/T-VAR/T-CONE/
  T-DET/H-SEL/N0.
- **Next, each requiring an explicit user commission (recommended order):**
  1. **Park decision** for the general T-DIR lane (a freeze-review-type
     ruling: PARK with the SSC record as the closing evidence; the family's
     proved arrows, Y-1/Y-2, DQ-1, V-15 survive as record theorems).
  2. **SYM-test of the recorded fallbacks HR-1 / HR-3′** (cheap, on paper):
     evaluate each on `𝔅_Γ(M)`; a head that reduces to a wall statement
     there is not sub-wall and should not be commissioned. Only a
     SYM-surviving head proceeds to its own counterexample-first battery.
  3. Standing alternatives unchanged: the `SPEC.md` numerical program with
     M-1 (on hold); the passive literature watch; SEL-3/SEL-5/EB-1 Lean
     debts (on hold).
- **Forbidden shortcuts (standing):** no asserting open Props as axioms; no
  citing conditional chains without hypotheses; no [H]-tagged citation
  consumption before first-hand fetch; local Elan-pinned gate only, direct
  fast-forward push to `main`, no PR; no claim about regularity or
  singularity of symmetric flows (SSC is an obstruction lemma, not a
  regularity or blow-up statement).

### (superseded) Next work (written 2026-09-03, fourteenth session)
...
verdict + the exact chain with EV-1 quantifiers);
`docs/gates/HSEL_V12_PRIMARY_AUDIT_2026-09-02.md` (§2 re-based head, §3
proposals); `docs/gates/HSEL_PROOF_ROUTE_SELECTION_2026-09-02.md` (§1
mechanism, with supersession notes); `docs/gates/HSEL_QUANTIFIER_AUDIT_2026-09-02.md`
(EQ-5/EQ-7); `docs/gates/TSEL_HEAD_REDUCTION_AUDIT_2026-09-02.md` (§9);
`docs/gates/STAGE9_REVERSE_GAP_AUDIT_2026-09-02.md` SS-5/SS-6;
`FORMAL_SCOPE.md` (T-SEL subsection).

- **Head lineage now:** general head = `H-SEL^nu` (p = 2, norm-uniform —
  OPEN; provably holds on the small-critical sub-ball, up to `T₀(ν,M)`,
  and on the axisym no-swirl sub-ball). **Operative source-control head:
  T-SRC′ = `‖u_θ/r‖_{L⁴_tL²_x} ≤ Q₀(ν,T,M,M′)`** on the axisym
  two-parameter ball (`M` = carrier `H³`, `M′ = ‖ru₀^θ‖_∞` — EV-1). **Its
  entire downstream chain is now verified, published, quantitative known
  mathematics** (Li–Pan Thm 1.3 at the native `(1,4,2)` endpoint; V-12 +
  V-13 discharged): T-SRC′ ⟹ `sup_t‖u‖_{H³} ≤ F(ν,T,M,M′,Q₀)` ⟹
  H-SEL^nu on the `(M,M′)`-ball / H-SEL^ds|axisym per-datum ⟹ N0|axisym.
  Only the head itself is open. Do not cite the retired `L⁴_t(L⁵∩L⁸)` form.
- **Next, each requiring an explicit user commission (recorded
  recommendation, in order):**
  1. **P-2 — the family probe, final observable set:** axisym-with-swirl
     `(M,M′)`-ball families at fixed `(ν,T)` measuring `Q₅`, the budget
     `∫‖u_θ/r‖₂⁴dt`, and the temporal profile `h(t) = ‖u_θ/r‖₂²`
     (falsification signature = temporal concentration of `h`); fail-closed
     protocol; finite-cylinder certificate-inadmissible; preregistered
     thresholds; EVIDENCE-FOR/AGAINST/UNDETERMINED only.
  2. **T-SRC′ proof commissioning** (counterexample-first discipline
     satisfied: P-1/P-3-level checks done for the lineage, battery run,
     bridge verified; the recorded refinement opportunity — avoiding the
     `J`-step's `Γ₀`-consumption for M-only uniformity — can ride along).
  3. **Freeze-review/watch proposals awaiting user adjudication:** P-LP
     ...
     class-wide kill; no un-park trigger fired) and the Shahmurov
     2605.01875/2605.09797 watch routing (same author as the gap-confirmed
     D-3).
- **Still separately uncommissioned — do not start implicitly:** any proof
